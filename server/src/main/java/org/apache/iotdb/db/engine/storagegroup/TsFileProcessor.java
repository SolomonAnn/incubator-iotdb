...
public class TsFileProcessor {
   ...
   */
  public boolean insert(InsertPlan insertPlan) throws PathErrorException, QueryProcessorException {
    Long deviceId;
  ...
  public boolean insertBatch(BatchInsertPlan batchInsertPlan, List<Integer> indexes,
      Integer[] results) throws PathErrorException, QueryProcessorException {
    if (workMemTable == null) {
      workMemTable = MemTablePool.getInstance().getAvailableMemTable(this);
    }
    ...
    try {
      ...
      for (IMemTable flushingMemTable : flushingMemTables) {
        ...
        ReadOnlyMemChunk memChunk = flushingMemTable
            .query(devicePath, measurementPath, dataType, props, context.getQueryTimeLowerBound());
        if (memChunk != null) {
